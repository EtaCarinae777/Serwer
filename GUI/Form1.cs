    ...
    public partial class Form1 : Form
    {
        ...
        private sealed class TcpConnectionPool : IDisposable
        {
            ...
            private static bool IsAlive(TcpClient c)
            {
                try
                {
                    if (c == null || !c.Connected || c.Client == null || !c.Client.Connected) return false;
        ...
        private async void btnAnalyzuj_Click(object sender, EventArgs e)
        {
                    ...
                    foreach (var adres in adresy)
                    {
                        foreach (var sciezka in posortowane)
                        {
                            ...
                            uploadTasks.Add(Task.Run(async () =>
                            {
                                ...
                                finally { uploadThrottle.Release(); }
                            }));
                        }
                    }
            ...
            await Task.Run(() =>
            {
                ...
                Parallel.ForEach(pary, opcje, (para) =>
                {
                    ...
                    try
                    {
                        var wynik = WyslijCompareNaKonkretnySerwer(
                            serwerAdres, fileIds, port, para.Item1, para.Item2, n, grainSize);
            ...
            lblStatus.Text = $"Gotowe! Przeanalizowano {wczytaneWyniki.Count} par." + (plagiatow > 0 ? $" Wykryto {plagiatow} plagiatow!" : "");
        ...
        // ── Zoptymalizowany wysyłacz (Faza 2) bez sztucznego zawieszania wątków
        private WynikPary WyslijCompareNaKonkretnySerwer(
            string adres, Dictionary<string, Dictionary<string, string>> fileIds,
            int port, string plikA, string plikB, int n, int grainSize)
        {
            if (!fileIds.ContainsKey(adres) ||
                !fileIds[adres].TryGetValue(plikA, out string idA) ||
                !fileIds[adres].TryGetValue(plikB, out string idB))
            {
                return null;
            }

            // Maksymalnie 2 szybkie próby per wywołanie
            for (int proba = 0; proba < 2; proba++)
            {
                ...
                if (proba == 0) Thread.Sleep(50); // Oddech w razie błędu łącza (zamiast dotychczasowych 300ms w pętli 6x)
            }
            return null;
        }
        ...
        private WynikPary WyslijCompareZPuli(
            string adres, int port, string fileIdA, string fileIdB,
        ...
        {
            FolderBrowserDialog dialog = new FolderBrowserDialog { Description = "Wybierz folder z wynikami (Raporty)" };
            if (dialog.ShowDialog() != DialogResult.OK) return;
            ...
            {
                MessageBox.Show("Nie znaleziono zadnych plikow JSON w tym folderze!", "Brak wynikow", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            foreach (string plikJson in plikiJson)
            {
                try
                {
                    ...
                    if (wynik == null || wynik.plikA == null || wynik.plikB == null) { bledy++; continue; }
            ...
            OdswiezListe();
            MessageBox.Show($"Wczytano {wczytaneWyniki.Count} wynikow." + (bledy > 0 ? $"\nPominieto {bledy} uszkodzonych plikow." : ""), "Wczytywanie zakonczone", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void ZapiszLog(string komunikat)
        {
            ...
            {
                Directory.CreateDirectory("Raporty");
                string path = Path.Combine("Raporty", "errors.log");
                lock (_logLock) File.AppendAllText(path, $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} {komunikat}{Environment.NewLine}");
            }
            catch { }
        }
    }

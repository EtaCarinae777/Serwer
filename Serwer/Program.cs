...
class Server
{
    ...
    static void ObsluzUpload(BinaryReader reader, BinaryWriter writer)
    {
        ...
        // Zapis do struktury bez locków
        tekstySurowe.TryAdd(fileId, tekst);
    ...
    // Bezpieczne kolekcje asynchroniczne
    static readonly ConcurrentDictionary<string, string> tekstySurowe = new ConcurrentDictionary<string, string>();
    static readonly ConcurrentDictionary<string, (HashSet<string> ngramy, Dictionary<string, (int od, int do_)> pozycje)> ngramCache
        = new ConcurrentDictionary<string, (HashSet<string>, Dictionary<string, (int, int)>)>();
    ...
    static void ObsluzCompare(BinaryReader reader, BinaryWriter writer)
    {
        ...
        lock (cacheLock)
        {
            if (!cache.ContainsKey(fileIdA)) { writer.Write(-1.0); writer.Flush(); return; }
            if (!cache.ContainsKey(fileIdB)) { writer.Write(-1.0); writer.Flush(); return; }
    ...
    // ── N-gramy (Eager Cache via GetOrAdd) ───────────────────────────────────
    static (HashSet<string> ngramy, Dictionary<string, (int od, int do_)> pozycje) PobierzNgramy(string fileId, int n)
    {
        ...
        return ngramCache.GetOrAdd(klucz, _ =>
        {
            ...
            return GenerujNgramy(tokeny, n);
        });
    }

    // ── Reszta operacji na stringach i matematyce ─────────────────────────────
    ...
    static (HashSet<string> ngramy, Dictionary<string, (int od, int do_)> pozycje) GenerujNgramy(List<(string slowo, int od, int do_)> tokeny, int n)
    {
    ...
    static (HashSet<string> intersection, double jaccard, double aDoB, double bDoA) ObliczStatystyki(HashSet<string> A, HashSet<string> B)
    {
        if (A.Count == 0 || B.Count == 0) return (new HashSet<string>(), 0, 0, 0);
        var (mniejszy, wiekszy) = A.Count <= B.Count ? (A, B) : (B, A);
        var intersection = new HashSet<string>();
        foreach (string s in mniejszy)
            if (wiekszy.Contains(s)) intersection.Add(s);
    ...
    static List<(int od, int do_)> ZnajdzPodobneZakresy(Dictionary<string, (int od, int do_)> pozycje, HashSet<string> wspolneNgramy)
    {
        var zakresy = new List<(int od, int do_)>(wspolneNgramy.Count);
        foreach (var ngram in wspolneNgramy)
            if (pozycje.TryGetValue(ngram, out var z)) zakresy.Add(z);
        ...
        foreach (var z in zakresy)
        {
            if (polaczone.Count == 0 || z.od > polaczone[polaczone.Count - 1].do_)
                ...
                polaczone[polaczone.Count - 1] = (polaczone[polaczone.Count - 1].od, Math.Max(polaczone[polaczone.Count - 1].do_, z.do_));
        }
        return polaczone;
    }

    static string GenerujCSV(string nazwaA, string nazwaB, double jaccard, double aDoB, double bDoA, List<(int od, int do_)> zakresy1, List<(int od, int do_)> zakresy2)
    {
    ...
    static string GenerujJSON(string nazwaA, string nazwaB, double jaccard, double aDoB, double bDoA, List<(int od, int do_)> zakresy1, List<(int od, int do_)> zakresy2)
    {
